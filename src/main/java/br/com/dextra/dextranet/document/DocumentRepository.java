...
public class DocumentRepository extends BaseRepository {
	...
	public void alteraDocumento(Comment comment) throws EntityNotFoundException {

		DatastoreService datastore = DatastoreServiceFactory
				.getDatastoreService();
		Key key = KeyFactory.createKey(Post.class.getName(), comment
				.getIdReference());
		...
		Document document = Document.newBuilder().setId(
				...
				Field.newBuilder().setName(
						"comment"
								+ e.getProperty(PostFields.COMENTARIO
										.getField()))
						.setHTML(comment.getText())).addField(
				Field.newBuilder().setName(
						PostFields.DATA_DE_ATUALIZACAO.getField()).setText(
						comment.getDataDeCriacao())).build();

		IndexFacade.getIndex(Post.class.getName()).add(document);
	}

	public void alteraDocumento(String id, String data) throws EntityNotFoundException {
